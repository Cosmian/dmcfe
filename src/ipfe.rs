...
pub fn setup(l: usize) -> (Vec<Scalar>, Vec<G1Affine>) {
    let msk: Vec<Scalar> = (0..l).map(|_| interface::random_scalar()).collect();
    let MPK: Vec<G1Affine> = (0..l).map(|i| interface::smul_in_g1(&msk[i])).collect();
    (msk, MPK)
}
...
pub fn encrypt(MPK: &[G1Affine], x: &[Scalar]) -> (G1Affine, Vec<G1Affine>) {
    ...
    let c0: G1Affine = interface::smul_in_g1(&r);
    let c: Vec<G1Affine> = x
        .iter()
        .zip(MPK.iter())
        .map(|(&xi, &hi)| G1Affine::from(interface::smul_in_g1(&xi) + (hi * r)))
        .collect();
    (c0, c)
}
