...
/// - `min`:      minimal size
/// - `max`:      maximal size
/// - `step`:     size of the step (multiplicative steps)
/// - `n_iter`:   number of iteration per size
pub fn benchmark_dlp(min: u64, max: u64, step: u64, n_iter: usize) {
    let mut m = min;

    while m <= max {
        let j: u32 = (m as f64).sqrt() as u32;
        let k: u32 = (m / (j as u64)) as u32;

        print!("Benchmark (M = {}, m = {}, n = {}): ", m, j, k);

        // iterate N_ITER for (j,k)
        let ms: u128 = (0..n_iter).map(|_i| get_time_dlp(j, k)).sum::<u128>() / (n_iter as u128);
